...
final class TransactionsCsvParser {
    private static final List<String> REQUIRED_COLUMNS = List.of("occurred_on", "direction", "amount", "currency");
    ...
    static List<ImportTransactions.Row> parse(MultipartFile file) {
        ...
        try (var reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8))) {
            ...
            if (headerLine == null || headerLine.trim().isEmpty()) throw new IllegalArgumentException("CSV file is empty");
    ...
    private static ImportTransactions.Row parseRow(String[] fields, Map<String, Integer> indexes, int lineNumber) {
        LocalDate occurredOn = parseOccurredOn(requiredField(fields, indexes, "occurred_on", lineNumber), lineNumber);
        TransactionDirection direction = parseDirection(requiredField(fields, indexes, "direction", lineNumber), lineNumber);
        BigDecimal amount = parseAmount(requiredField(fields, indexes, "amount", lineNumber), lineNumber);
        Currency currency = parseCurrency(requiredField(fields, indexes, "currency", lineNumber), lineNumber);
        String memo = optionalField(fields, indexes, "memo");

        return new ImportTransactions.Row(occurredOn, direction, amount, currency, memo);
    }

    private static String requiredField(String[] fields, Map<String, Integer> indexes, String column, int lineNumber) {
        Integer index = indexes.get(column);
        if (index == null || index >= fields.length) {
            throw new IllegalArgumentException("Row " + lineNumber + " is missing required column '" + column + "'");
        }

        String value = fields[index].trim();
        if (value.isEmpty()) {
            throw new IllegalArgumentException("Row " + lineNumber + " has empty value for '" + column + "'");
        }
        return value;
    }

    private static String optionalField(String[] fields, Map<String, Integer> indexes, String column) {
        Integer index = indexes.get(column);
    ...
    private static LocalDate parseOccurredOn(String value, int lineNumber) {
        try {
            ...
            throw new IllegalArgumentException("Row " + lineNumber + " has invalid occurred_on '" + value + "'");
        }
    }
    ...
    private static TransactionDirection parseDirection(String value, int lineNumber) {
        try {
            ...
            throw new IllegalArgumentException("Row " + lineNumber + " has invalid direction '" + value + "'");
        }
    }
    ...
    private static BigDecimal parseAmount(String value, int lineNumber) {
        try {
            ...
            throw new IllegalArgumentException("Row " + lineNumber + " has invalid amount '" + value + "'");
        }
    }
    ...
    private static Currency parseCurrency(String value, int lineNumber) {
        try {
            ...
            throw new IllegalArgumentException("Row " + lineNumber + " has invalid currency '" + value + "'");
        }
    }
