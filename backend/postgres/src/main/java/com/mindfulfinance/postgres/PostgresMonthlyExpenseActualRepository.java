...
public final class PostgresMonthlyExpenseActualRepository
    implements MonthlyExpenseActualRepository {
  private static final Currency RUB = Currency.getInstance("RUB");

  private static final RowMapper<MonthlyExpenseActual> ROW_MAPPER =
      (rs, rowNum) -> {
        Map<PersonalExpenseCategory, Money> amounts = new EnumMap<>(PersonalExpenseCategory.class);
        amounts.put(
            PersonalExpenseCategory.RESTAURANTS, new Money(rs.getBigDecimal("restaurants"), RUB));
        amounts.put(
            PersonalExpenseCategory.GROCERIES, new Money(rs.getBigDecimal("groceries"), RUB));
        amounts.put(PersonalExpenseCategory.PERSONAL, new Money(rs.getBigDecimal("personal"), RUB));
        amounts.put(
            PersonalExpenseCategory.UTILITIES, new Money(rs.getBigDecimal("utilities"), RUB));
        amounts.put(
            PersonalExpenseCategory.TRANSPORT, new Money(rs.getBigDecimal("transport"), RUB));
        amounts.put(PersonalExpenseCategory.GIFTS, new Money(rs.getBigDecimal("gifts"), RUB));
        amounts.put(
            PersonalExpenseCategory.INVESTMENTS, new Money(rs.getBigDecimal("investments"), RUB));
        amounts.put(
            PersonalExpenseCategory.ENTERTAINMENT,
            new Money(rs.getBigDecimal("entertainment"), RUB));
        amounts.put(
            PersonalExpenseCategory.EDUCATION, new Money(rs.getBigDecimal("education"), RUB));
