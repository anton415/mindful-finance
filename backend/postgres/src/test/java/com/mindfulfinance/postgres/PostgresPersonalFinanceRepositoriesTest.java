...
public class PostgresPersonalFinanceRepositoriesTest {
  ...
  void repositories_are_card_scoped_and_support_upsert_lookup_and_delete() {
    PersonalFinanceCardId firstCardId =
        new PersonalFinanceCardId(UUID.fromString("4fd714c7-52eb-49e1-9def-74666757f8d0"));
    PersonalFinanceCardId secondCardId =
        new PersonalFinanceCardId(UUID.fromString("7f2353d6-6b27-4fe6-9c13-5151605bcba8"));
    AccountId firstLinkedAccountId =
        new AccountId(UUID.fromString("1d0aef39-a0ba-4fe7-8b34-82d5d727c41a"));
    AccountId secondLinkedAccountId =
        new AccountId(UUID.fromString("ac03f92b-f0f4-41e4-a302-a8b1cb15e17d"));
    ...
    expenseActualRepository.upsert(
        new MonthlyExpenseActual(
            firstCardId,
            2026,
            2,
    ...
    expenseLimitRepository.upsert(
        new MonthlyExpenseLimit(
            firstCardId, Map.of(PersonalExpenseCategory.RESTAURANTS, new BigDecimal("50.00"))));
    incomeActualRepository.upsert(
        new MonthlyIncomeActual(firstCardId, 2026, 3, new Money(new BigDecimal("1400.00"), RUB)));
    incomeForecastRepository.upsert(
        new IncomeForecast(
            firstCardId, new Money(new BigDecimal("1000.00"), RUB), new BigDecimal("20.00")));
    incomePlanRepository.upsert(
        new IncomePlan(
            firstCardId,
            2026,
            java.util.List.of(
    ...
    assertThat(cardRepository.find(secondCardId))
        .get()
        .extracting(PersonalFinanceCard::status)
    ...
    assertThat(expenseActualRepository.findByCardAndYear(firstCardId, 2026).get(0).month())
        .isEqualTo(1);
    assertThat(
            expenseActualRepository
                .findByCardAndYear(firstCardId, 2026)
                .get(1)
                .categoryAmounts()
    ...
    assertThat(
        ...
        .isEqualTo(1);
    assertThat(incomePlanRepository.findByCardAndYear(firstCardId, 2026).orElseThrow().vacations())
        .hasSize(1);
