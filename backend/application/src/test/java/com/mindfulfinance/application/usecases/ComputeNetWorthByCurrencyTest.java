...
public class ComputeNetWorthByCurrencyTest {
  ...
  void testComputeNetWorthByCurrency() {
    ...
    Account account1 =
        new Account(
            accountId1, "Test Account", Currency.getInstance("USD"), CASH, ACTIVE, Instant.now());
    ...
    Account account2 =
        new Account(
            accountId2, "Test Account 2", Currency.getInstance("EUR"), CASH, ACTIVE, Instant.now());
    ...
    assertEquals(
        new Money(new BigDecimal("100.00"), Currency.getInstance("USD")),
        computeNetWorth.compute().get(Currency.getInstance("USD")));
    assertEquals(
        new Money(new BigDecimal("50.00"), Currency.getInstance("EUR")),
        computeNetWorth.compute().get(Currency.getInstance("EUR")));
  }
  ...
  void testTwoUSDAccountsAggregateIntoOneUSDBucket() {
    ...
    Account account1 =
        new Account(
            accountId1, "Test Account 1", Currency.getInstance("USD"), CASH, ACTIVE, Instant.now());
    accounts.save(account1);
    Transaction tx1 =
        new Transaction(
            TransactionId.random(),
    ...
    Account account2 =
        new Account(
            accountId2, "Test Account 2", Currency.getInstance("USD"), CASH, ACTIVE, Instant.now());
    accounts.save(account2);
    Transaction tx2 =
        new Transaction(
            TransactionId.random(),
