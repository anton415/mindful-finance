...
/**
 * Use case for computing the net worth by currency by summing up the balances of all accounts
...
public final class ComputeNetWorthByCurrency {
  ...
  /**
   * Computes the net worth by currency by summing up the balances of all accounts grouped by their
   * currency.
   *
  ...
  public Map<Currency, Money> compute() {
    ...
    // Iterate through all accounts and compute their balances, then aggregate the totals by
    // currency
    for (Account account : accounts.findAll()) {
      ...
      // Merge the balance into the totals map, summing it with any existing total for the same
      // currency
