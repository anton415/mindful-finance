...
public record VacationPeriod(
    ...
    public VacationPeriod {
        if (startDate == null) {
            throw new DomainException(
                ...
                "Vacation period start date must not be null",
                null
            );
        }
        if (endDate == null) {
            throw new DomainException(
                ...
                "Vacation period end date must not be null",
                null
            );
        }
    ...
    public boolean overlapsOrTouches(VacationPeriod other) {
        return !other.startDate().isAfter(endDate.plusDays(1)) && !other.endDate().isBefore(startDate.minusDays(1));
    }
