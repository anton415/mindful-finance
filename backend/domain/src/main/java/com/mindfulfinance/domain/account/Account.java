...
/**
 * Represents a financial account with various attributes such as name, currency, type, status, and
 * creation timestamp. This class is immutable and thread-safe.
 */
public record Account(
    ...
    Instant createdAt) {
  /**
   * Constructs a new Account instance with the given attributes, performing validation to ensure
   * that all required fields are provided and valid.
   *
  ...
  public Account {
    ...
    if (name == null || name.isBlank()) {
      throw new DomainException(
          ACCOUNT_NAME_NULL_OR_BLANK, "Account name cannot be null or blank", null);
    }
    if (currency == null) {
  ...
  /**
   * Returns a new Account instance with the status set to ARCHIVED, effectively marking the account
   * as closed.
   *
