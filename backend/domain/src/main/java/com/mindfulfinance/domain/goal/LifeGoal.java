...
/**
 * Represents a life goal that a user is working towards, such as saving for a down payment on a
 * house or funding a child's education. This class is immutable and thread-safe.
 */
public record LifeGoal(
   ...
   * @param targetAmount the target amount of money needed to achieve the life goal, must not be
   *     null, negative, or zero
   * @param targetDate the target date by which the life goal should be achieved, must not be null
   * @param status the current status of the life goal (e.g., ACTIVE, ACHIEVED, ARCHIVED), must not
   *     be null
  ...
  public LifeGoal {
    ...
    if (title == null || title.isBlank()) {
      throw new DomainException(
          LIFEGOAL_TITLE_NULL_OR_BLANK, "LifeGoal title cannot be null or blank", null);
    }
    if (targetAmount == null || targetAmount.isNegative() || targetAmount.isZero()) {
      throw new DomainException(
          LIFEGOAL_TARGET_AMOUNT_NULL_OR_NEGATIVE_OR_ZERO,
          "LifeGoal targetAmount cannot be null, negative, or zero",
          null);
    }
    if (targetDate == null) {
      throw new DomainException(
          LIFEGOAL_TARGET_DATE_NULL, "LifeGoal targetDate cannot be null", null);
    }
    if (status == null) {
    ...
    if (createdAt == null) {
      throw new DomainException(
          LIFEGOAL_CREATED_AT_NULL, "LifeGoal createdAt cannot be null", null);
    }
  }
