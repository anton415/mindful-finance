...
- **Done when:**
  ...
  - The repo has a documented path for manual deploy/smoke automation that can be connected to the hybrid remote-access deployment later.

## Summary (scope)
- **In scope:** CI baseline, dependency/security bots, architecture guard automation, and manual automation hooks for remote-access operations.
...
- Milestone 12 expects a repeatable deployment path and smoke checks for the home-server + Cloudflare baseline, but does not require full production CD yet.
...
- Manual `workflow_dispatch` is the right first step for remote-access deploy/smoke automation before any auto-deploy policy exists.
...
- Should post-deploy smoke checks stop at `/api/health`, or also include the Cloudflare Access gate and one CRUD path once the hybrid deploy exists?
...
5) **Prepare deploy automation hooks**
- Add manual deploy and post-deploy smoke workflows once the home-server + Cloudflare baseline is ready.
- Keep them explicit and operator-driven at first.
- Checkpoint: manual dry-run succeeds against the remote-access environment without hidden local steps.
...
- [ ] Manual deploy/smoke automation tasks are tracked for the remote-access path.
...
- Add staging deploy automation after the first remote-access path is operational.
...
- Decision (2026-03-10): `main` governance now targets one required approval plus required checks `Backend tests`, `Frontend lint and build`, `Framework boundary`, `Analyze (java-kotlin)`, and `Analyze (javascript-typescript)`.
...
- Decision (2026-03-10): `dependency-review` stays non-required until repository security settings support it reliably; otherwise branch protection would deadlock normal PR merges.
