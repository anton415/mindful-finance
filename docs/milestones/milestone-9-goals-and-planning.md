...
- Decision (2026-03-14): personal-finance card archive and hard delete are separate actions. `Archive` preserves card-scoped data and synthetic linked-account transactions, archives the linked `CASH` account, hides that account from the standard Investments list, and makes the card read-only until restore.
